...
export async function createProducto(producto: Partial<IProducto>): Promise<IProducto> {
  const { nombre, precio, cantidad, foto, valor_compra_usd } = producto;
  const result = await query(
    'INSERT INTO productos (nombre, precio, cantidad, foto, valor_compra_usd) VALUES ($1, $2, $3, $4, $5) RETURNING *',
    [nombre, precio, cantidad, foto, valor_compra_usd]
  );
...
export async function updateProducto(id: string, producto: Partial<IProducto>): Promise<IProducto | null> {
  const { nombre, precio, cantidad, foto, valor_compra_usd } = producto;
  const result = await query(
    'UPDATE productos SET nombre = $1, precio = $2, cantidad = $3, foto = $4, valor_compra_usd = $5 WHERE id = $6 RETURNING *',
    [nombre, precio, cantidad, foto, valor_compra_usd, id]
  );
  return result.rows[0] || null;
}
