...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      // Si el destino es un Vendedor, registrar/iniciar vigencia
      try {
        const esVendedorRes = await query(`SELECT rol FROM usuarios WHERE id = $1`, [vendedorId]);
        ...
        console.warn('Advertencia al registrar vigencia:', vigErr);
      }
